...
app.get("/filter", (req, res)=>{
  ...
  const foundType = jokes.filter((joke) => joke.jokeType === type);
  res.json(foundType);
});
//4. POST a new joke
...
app.delete("/jokes/:id", (req,res)=>{
  const id = parseInt(req.params.id );
  const foundJokeId = jokes.findIndex((joke) =>joke.id === id);
  const deleteJoke = jokes.splice(foundJokeId, 1);
  res.sendStatus(200);
});
//8. DELETE All jokes
app.delete("/all", (req,res)=>{
  ...
  if(userKey === masterKey){
    ...
    res
    .status(404)
    .json({error : "why are you trying to delte this 😱 Go away!"});
  }
});
app.listen(port, () => {
